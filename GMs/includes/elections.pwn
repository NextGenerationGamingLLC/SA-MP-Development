...
LoadElections() {
	...
	SetGVarInt("ElectionArea", DynamicArea:CreateDynamicSphere(366.54, 159.09, 1008.38, 100));
	return 1;
}

UnloadElections() {
	
	DestroyDynamicArea(DynamicArea:GetGVarInt("ElectionArea"));
	DeleteGVar("ElectionArea");
}

hook OnPlayerEnterDynamicArea(playerid, areaid) {

	if(areaid == DynamicArea:GetGVarInt("ElectionArea")) SendClientMessage(playerid, COLOR_WHITE, "You have entered an election area - press ~k~~CONVERSATION_YES~ to vote");
	return 1;
}

hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {

	if(newkeys & KEY_YES && IsPlayerInDynamicArea(playerid, DynamicArea:GetGVarInt("ElectionArea"))) {
